...
def retry_once(gen, rec, pool, seeds, max_new_tokens, mode="typed", budget=8):
    ...
    trace = G._ensure_hash_line(trace, G.extract_final_answer(trace))
...
def run_full(gen, manual, eval_problems, strategy_id, n_problems,
    ...
    print(f"#  RETRY MODE = {retry_mode.upper()}  "
          f"({'typed positives + typed instruction [FADE]' if retry_mode=='typed' else 'untyped positives, no instruction [generic ablation]' if retry_mode=='generic' else 'blind re-sample, no diagnosis [immediate null]'})")
