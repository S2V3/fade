...
def version_banner() -> None:
    ...
    if v != "v3-answerstop":
        print("  !! expected 'v3-answerstop' -- generation.py is stale/modified. Abort.")
        sys.exit(1)
    print(f"  v3-answerstop ACTIVE | N_SHOTS={N_SHOTS} "
...
def _show_pass1_trace(i, n, q, cat_info, trace, ntok, dt, gold_ans, comps, label, diag):
    """Print the actual reasoning + scoring for one problem, live during the run,
    so quality problems (loops, mis-extraction, garbage) are visible mid-run."""
    print("\n" + "-" * 70)
    print(f"[{i}/{n}] {cat_info['category_type']}/{cat_info['complexity']} | "
          f"correct={comps.correct} | {label.value}"
          + (f" / {diag.ftype.value}" if diag else ""))
    print(f"Q: {q[:110]}{'...' if len(q) > 110 else ''}")
    print(f"TRACE ({ntok} tok, {dt:.1f}s):")
    print("\n".join("  " + ln for ln in trace.splitlines()) or "  <empty>")
    print(f"pred={comps.final_answer} gold={gold_ans} | "
          f"E={comps.E:.2f} V={comps.V:.2f} R={comps.R:.2f} G={comps.G:.2f} "
          f"coherent={comps.coherent} | misses={comps.misses} bad_eqs={comps.bad_eqs}")
    print("-" * 70)
...
def run_full(gen, manual, eval_problems, strategy_id, n_problems,
    ...
    for i, prob in enumerate(problems, 1):
        ...
        if show_every and (i % show_every == 0):
            _show_pass1_trace(i, len(problems), q, cat_info, trace,
                              res["num_tokens"], dt, gold_ans, comps, label, diag)
