...
when kaggle_run.py disagrees with your hand label you can see exactly WHY --
...
def main(data_path: Path, item_id: str, r_variant: str, margin: float):
    ...
    if config.NORMALIZE_TRACES:            # match kaggle_run.score_trace
        tr = normalize_math(tr)
    ...
    if needs_diagnosis(label):
        d = diagnose(c.E, c.V, c.n_steps, c.n_checkpoints,
                     c.G, c.coherent, n_equations=c.n_equations, margin=margin)
