...
Ordered cascade (v5):
  ...
  2. ST -- step omission: s < 0.6*s_hat AND s_hat >= ST_MIN_SHAT AND E is
     ALSO low (E < ST_E_MAX). The E gate is the fix for the long-standing
     ST-over-fires bug: previously ANY short trace was ST regardless of E,
     so terse-but-faithful CE traces were mislabelled ST and CE was starved
     (which then poisons the FST label it trains on). Genuine step omission
     misses checkpoints, so it shows low E; a short trace that still hit
     most checkpoints is compact-correct work and now falls through (to
     abstain/UNCLASSIFIED -> generic, a safe default) instead of getting the
     wrong typed cure. Prefer an A-based bound here once diagnose() is passed
     A; E is what this function currently receives. NR-first still guarantees
     "real work" before ST is considered.
...
def diagnose(E: float, V: float, s: int, s_hat: int,
    ...
    # 2 -- ST: real-but-compressed work, but only when execution fidelity is
    #      ALSO low. Genuine step omission misses checkpoints (low E); a short
    #      trace that still hit most checkpoints is compact-correct work, not
    #      omission, and must not be routed to the decomposition cure (that
    #      starves CE and poisons the FST label). Prefer an A-based bound once
    #      diagnose() is passed A; E is what this function currently receives.
