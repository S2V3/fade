...
from config import STEP_SPLIT_SENTENCES
...
def split_steps(trace: str) -> list[str]:
    ...
    if STEP_SPLIT_SENTENCES:
        # Sentence-split EVERY line, not only a lone one. Terse chat traces pack
        # several computational steps onto one line; pure line-counting then
        # under-reports s and the cascade reads it as step omission (the ST
        # root cause). OFF by default -> behaviour identical to before; enable
        # and validate on dev. NOTE: this also re-segments R's step list.
        out = []
        for ln in lines:
            sents = [x.strip() for x in RE_SENTENCE_SPLIT.split(ln) if x.strip()]
            out.extend(sents if sents else [ln])
        return out or lines
    if len(lines) == 1:
