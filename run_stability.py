...
def analyze(path: Path):
    ...
    print("=" * 72)
    if kk != kk:
        print("UNDEFINED -- not enough wrong samples. Increase --n or --k.")
    elif kk < 0.20:
        print(f"UNSTABLE (kappa={kk:.3f}). The failure type is close to a coin flip "
              f"on re-sampling the SAME question, so no question-only feature can "
              f"predict it. The gate's NO-GO is a property of the phenomenon, not "
              f"of your feature set. Write the null: report this kappa as the "
              f"ceiling, and do NOT build fst_predictor.py.")
    elif kk < 0.40:
        print(f"WEAKLY STABLE (kappa={kk:.3f}). There is some reproducible signal "
              f"but it is fair at best. A predictor could in principle reach ~{kk:.2f} "
              f"agreement -- report the ceiling alongside the gate, and only build "
              f"the predictor if that margin is worth the claim.")
    else:
        print(f"STABLE (kappa={kk:.3f}). The type IS reproducible, so the gate's "
              f"NO-GO points at weak FEATURES, not an unpredictable target. Improve "
              f"the features (the categorizer collapses to 5 of its 8 values) and "
              f"re-run fst_gate.py before writing any null.")
    print("\nCAVEAT: measured at T={:.2f}. Stability is temperature-dependent; a "
          "greedy (T=0) run is deterministic and would trivially give kappa=1, "
          "which is why this uses the same sampling regime a real deployment "
          "would.".format(rows[0]["temperature"]))
