...
STRATEGY_NAMES: Dict[int, str] = {
    ...
    15: 'full_pipeline',
    16: 'fade_best',      # 8-shot CoT (KNN) + self-consistency maj@5 -- the
                          # strongest single arm; exploits the best-of-N gap
}
...
_HEADER = (
    ...
    "7. You MUST end your entire response with the final answer on its own "
    "line in exactly this format:\n"
    "#### <number>\n"
    "where <number> is a plain number with no units, no currency symbol, no "
    "words, and nothing after it. This '####' line is REQUIRED and must be the "
    "very last line. Do not write anything after it.\n\n"
)
...
GENERATION_VERSION = "v5-hashline"  # kaggle_run's guard checks this
...
# EXEMPLAR BUDGET (count parity across arms).
# For a fair comparison, every arm that shows exemplars must show the SAME
# number -- otherwise a gain could be "more shots help" rather than "better
# selection helps". When EXEMPLAR_BUDGET is set (not None), it OVERRIDES each
# strategy's own k, so few_shot, cot_few_shot, fade_best, generic-retry and
# typed-retry all show exactly EXEMPLAR_BUDGET exemplars. What varies between
# arms is then only WHICH exemplars and HOW they are chosen -- never how many.
EXEMPLAR_BUDGET = None                # e.g. set to 8 to force parity


def set_exemplar_budget(k):
    global EXEMPLAR_BUDGET
    EXEMPLAR_BUDGET = k
STOP_MARKERS = ["\nQuestion:", "\nQ:", "\nExample", "\nProblem:"]
...
def _to_chat_prompt(tokenizer, prompt: str, extra_system: str = "") -> str:
    ...
    effective than an 8-example wall of text in a single user message -- and it
    lets the assistant turns demonstrate the '#### <number>' ending in the exact
    position the model will generate into.
    ...
    # exemplars -> user/assistant turn pairs
    last_end = 0
    for m in _EX_BLOCK_RE.finditer(body):
        q, sol = m.group(1).strip(), m.group(2).strip()
        if q and sol:
    ...
    try:
        ...
        try:                       # some templates reject a system role
            merged = [{"role": "user",
                       "content": system + "\n\n" + msgs[-1]["content"]}]
            return tokenizer.apply_chat_template(
...
def _ensure_hash_line(trace: str, answer) -> str:
    """If the trace has no '#### N' line but the ladder found an answer, append a
    canonical '#### <answer>' line. Never changes the answer (it is the ladder's
    own result) -- only the surface format, so '####' coverage and coherence
    reflect the real answer instead of penalising prose phrasing."""
    if answer is None:
        return trace
    if re.search(r"#{3,}\s*-?\d", trace):          # already has a marker
        return trace
    # answer may be a float (single-seq) or a str (majority_vote). Coerce safely.
    try:
        val = float(answer)
    except (TypeError, ValueError):
        return trace                               # non-numeric -> leave as is
    num = str(int(val)) if val.is_integer() else ("%g" % val)
...
def _build_prompt(
    ...
    def _dyn(k=3):
        """Get k exemplars for this problem. When EXEMPLAR_BUDGET is set it
        overrides k so every arm is count-matched. Draws from the accumulated
        pool via knn_fn when available, otherwise from the seeds -- but always
        returns exactly k (topped up from seeds if the pool is too small, so a
        near-empty pool early in a run still yields a full, fair prompt)."""
        ...
        if len(chosen) < k:                     # top up from seeds to hit budget
            have = {id(e) for e in chosen}
            for e in manual_exemplars:
                if id(e) not in have:
                    chosen.append(e)
                    if len(chosen) >= k:
                        break
    ...
    if sid == 0:
        ...
        prompt = (
            f"{_HEADER}"
            f"{_fmt_exemplars(manual_exemplars, EXEMPLAR_BUDGET or N_SHOTS)}"
            f"Question: {problem}\nSolution:"
        )
    ...
    # GROUP 5 — HYBRID
        ...
        exs = _dyn(k=EXEMPLAR_BUDGET or N_SHOTS)
        prompt = (
            f"{_HEADER}Examples with step-by-step reasoning:\n\n"
            f"{_fmt_exemplars(exs, len(exs))}"
        ...
        # fade_best: strongest single arm. Dynamic 8-shot exemplars (KNN /
        # 3-stage when a knn_fn is provided) rendered as CoT, + self-consistency
        # majority vote over 5 samples. Directly exploits the best-of-N
        # elicitation gap (base LLaMA-2 ~14% greedy vs 97.7% best-of-256).
        exs = _dyn(k=N_SHOTS)
        prompt = (
            f"{_HEADER}"
            f"Here are worked examples. Solve the last question the same way, "
            f"showing every step and ending with #### <number>:\n\n"
            f"{_fmt_exemplars(exs, N_SHOTS)}"
            f"Question: {problem}\nSolution:"
        )
        gen_kw['num_return_sequences'] = 5
        gen_kw['temperature']          = 0.8
...
class ExemplarGenerator:
    ...
    def generate(
        ...
        # Format normalisation: if the model reached an answer but didn't emit a
        # clean '#### N' line (common with chat models that end in prose), append
        # the canonical marker derived from the SAME ladder used for scoring. This
        # does NOT change the answer -- it only makes the stored trace carry the
        # standard format, so '####' coverage and coherence reflect the real
        # answer instead of penalising prose phrasing.
        best_trace = _ensure_hash_line(best_trace, best_ans)
