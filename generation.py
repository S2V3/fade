...
def _to_chat_prompt(tokenizer, prompt: str) -> str:
    ...
    # whatever remains is the live question (plus any typed instruction line)
    tail = body[last_end:].strip()
    tail = re.sub(r"^Question:\s*", "", tail)
    tail = re.sub(r"\n?Solution:\s*$", "", tail).strip()
    msgs.append({"role": "user", "content": tail or body.strip()})
