"""
Two responsibilities, deliberately together (the label IS the routing):
  1. classify()   -- the count-based label table
  2. TraceStore   -- persistent storage that routes by label:
        GOOD      -> store/pool.jsonl        (the Phase-1 exemplar pool;
                                              the ONLY way in)
        MEDIUM_*  -> store/medium_queue.jsonl (retried FIRST, end-of-pass)
        BAD_*     -> store/bad_queue.jsonl    (retried after all MEDIUMs)

Three tiers, and a correct answer is NEVER "bad":
    GOOD    = correct AND clean            (the pool)          -> correct only
    MEDIUM  = correct-but-weak OR near-miss-wrong (the middle) -> mixed
    BAD     = wrong AND severe                                 -> wrong only

The label table:
    correct   misses=0 AND bad_eqs=0 AND coherent AND R>=0.6  -> GOOD
    correct   anything else                                   -> MEDIUM_CORRECT
    wrong     misses<=1 AND bad_eqs<=1 AND coherent AND G>=0.5 -> MEDIUM_WRONG
    wrong     anything else                                   -> BAD_WRONG

GOOD = perfect reasoning and perfect answer. MEDIUM = a correct trace that
isn't clean enough to teach from, OR a wrong trace that came close. BAD =
wrong answers with no redeeming reasoning. coherent=None (no equations)
counts as NOT coherent -- "no visible work" is not "coherent work".

The MEDIUM_CORRECT / MEDIUM_WRONG split is kept as sub-labels (both route to
the MEDIUM queue) ON PURPOSE: the honest recoverability comparison is
MEDIUM_WRONG vs BAD_WRONG -- both start wrong, so it is a fair test. Do NOT
compare the whole MEDIUM tier against BAD for "recovery": MEDIUM contains
already-correct traces and that inflates it for a non-reason.

Retry rule -- universal, "retry once, graduate or die": every non-GOOD
trace is diagnosed by the cascade and retried exactly once, end-of-pass,
MEDIUM queue before BAD queue. Successful MEDIUM retries enter the pool
BEFORE any BAD retry runs -- easy recoveries finance hard ones.

Phase-1 hook: TraceStore.exemplars() returns the GOOD pool entries
SANITISED of gold (question/trace/complexity/signals/embedding, but never
gold_solution/gold_answer) -- exactly what retrieval will consume, with the
reference solution physically removed so it cannot leak into a prompt.
"""
from __future__ import annotations

import json
import time
from enum import Enum
from pathlib import Path
from typing import Optional

from config import (GOOD_R_MIN, MEDIUM_MISSES_MAX, MEDIUM_WRONG_BAD_EQS_MAX,
                    G_MIN, STORE_ROOT, POOL_FILE, MEDIUM_QUEUE_FILE,
                    BAD_QUEUE_FILE)
from components import ComponentScores
from similarity import embed, backend_name

# ============================================================================
# GOLD HYGIENE
# ============================================================================
# Fields that carry the reference solution/answer. These are persisted on
# disk (for audit via load_pool) but MUST NOT ride along on any record that
# is handed to retrieval or rendering as an exemplar. Stripping them at the
# accessor makes the exemplar path gold-free by construction -- independent
# of which fields a downstream renderer happens to read.
_GOLD_FIELDS = ("gold_solution", "gold_answer")


def _sanitize_exemplar(record: dict) -> dict:
    """Copy of a pool record with gold removed. Retrieval and rendering only
    need question, trace, complexity, signals, embedding and id -- none of
    which is gold -- so this is a strict tightening with no downstream loss.
    Anything that legitimately needs the gold (audit, re-scoring) must read
    load_pool() directly instead of exemplars()."""
    return {k: v for k, v in record.items() if k not in _GOLD_FIELDS}


# Public alias: other modules (e.g. the harness) build exemplar records of their
# own and must strip gold before appending them to the prompting pool, using the
# exact same rule. Import this rather than reinventing the field list.
sanitize_exemplar = _sanitize_exemplar


# ============================================================================
# LABELS
# ============================================================================
class Label(str, Enum):
    GOOD = "GOOD"
    MEDIUM_CORRECT = "MEDIUM_CORRECT"
    BAD_CORRECT = "BAD_CORRECT"      # DEPRECATED: never emitted by classify().
                                     # Retained so importers / stored records /
                                     # the harness don't break. A correct trace
                                     # can no longer be BAD -- see classify().
    MEDIUM_WRONG = "MEDIUM_WRONG"
    BAD_WRONG = "BAD_WRONG"


CONSEQUENCE = {
    Label.GOOD: "enters exemplar pool (the only way in)",
    Label.MEDIUM_CORRECT: "diagnose -> typed polish retry (MEDIUM queue); pooled only if retry is GOOD",
    Label.BAD_CORRECT: "DEPRECATED -- correct traces are now MEDIUM_CORRECT, never BAD",
    Label.MEDIUM_WRONG: "diagnose -> typed hint-free retry (MEDIUM queue; the core case)",
    Label.BAD_WRONG: "diagnose -> typed hint-free retry (BAD queue, after all MEDIUM retries)",
}

# strict retry order: the MEDIUM queue drains before the BAD queue.
# BAD_CORRECT stays listed for backward-compat but classify() never emits it,
# so it simply matches nothing.
RETRY_ORDER = [Label.MEDIUM_WRONG, Label.MEDIUM_CORRECT,
               Label.BAD_WRONG, Label.BAD_CORRECT]


def classify(c: ComponentScores) -> Label:
    coh = bool(c.coherent)  # None -> False
    if c.correct:
        if c.misses == 0 and c.bad_eqs == 0 and coh and c.R >= GOOD_R_MIN:
            return Label.GOOD
        # A correct answer is NEVER bad: the worst a correct trace can be is a
        # weak-but-correct MEDIUM. (This absorbs the retired BAD_CORRECT bucket,
        # including "lucky guess" correct traces -- messy, low-checkpoint, or
        # incoherent-but-right. They still can only enter the pool if their
        # retry comes back clean GOOD, so the pool invariant is unaffected.)
        return Label.MEDIUM_CORRECT
    # wrong answer: never pooled, at any quality
    if (c.misses <= MEDIUM_MISSES_MAX and c.bad_eqs <= MEDIUM_WRONG_BAD_EQS_MAX
            and coh and c.G >= G_MIN):
        return Label.MEDIUM_WRONG
    return Label.BAD_WRONG


def is_pool_admissible(label: Label) -> bool:
    """Invariant: the pool contains exclusively GOOD traces."""
    return label is Label.GOOD


def needs_diagnosis(label: Label) -> bool:
    """EVERY non-GOOD trace runs the cascade -- weak successes included."""
    return label is not Label.GOOD


# ============================================================================
# PERSISTENT STORE
# ============================================================================
class TraceStore:
    """Append-only JSONL storage routed by label, deduped by trace id.

    Layout under `root` (default: ./store):
        pool.jsonl          GOOD only -- the growing exemplar pool
        medium_queue.jsonl  MEDIUM_CORRECT + MEDIUM_WRONG -- first retry queue
        bad_queue.jsonl     BAD_WRONG -- second retry queue

    Every record carries the full (question, trace, gold, answer), the label,
    the cascade diagnosis (for non-GOOD: it selects the typed cure at retry
    time), all component signals, and complexity = s_hat. Pool records
    additionally carry a precomputed question embedding when the MiniLM
    backend is active (TF-IDF vectors are not comparable across calls, so
    under the fallback the embedding is left null and Phase 1 recomputes).

    NOTE: the full record (incl. gold) is what is written to disk, for audit
    and re-scoring. The gold is stripped only at exemplars() -- the accessor
    retrieval/rendering consume -- so it cannot leak into a prompt.
    """

    def __init__(self, root: Path = STORE_ROOT):
        self.root = Path(root)
        self.root.mkdir(parents=True, exist_ok=True)
        self.paths = {
            "pool": self.root / POOL_FILE,
            "medium": self.root / MEDIUM_QUEUE_FILE,
            "bad": self.root / BAD_QUEUE_FILE,
        }

    # ------------------------------------------------------------- routing
    @staticmethod
    def _bucket(label: Label) -> str:
        if label is Label.GOOD:
            return "pool"
        if label in (Label.MEDIUM_CORRECT, Label.MEDIUM_WRONG):
            return "medium"
        return "bad"

    # ------------------------------------------------------------- writing
    def add(self, item_id: str, question: str, trace: str,
            gold_solution: str, gold_answer: float,
            label: Label, comps: ComponentScores,
            diagnosis: Optional[str] = None,
            diagnosis_reason: Optional[str] = None,
            diagnosis_confidence: Optional[float] = None) -> Optional[Path]:
        """Persist one classified trace. Returns the file written, or None
        if this id is already stored in that bucket (idempotent re-runs)."""
        bucket = self._bucket(label)
        path = self.paths[bucket]
        if item_id in self._ids(path):
            return None
        record = {
            "id": item_id,
            "question": question,
            "trace": trace,
            "gold_solution": gold_solution,
            "gold_answer": gold_answer,
            "label": label.value,
            "diagnosis": diagnosis,            # None for GOOD
            "diagnosis_reason": diagnosis_reason,
            "diagnosis_confidence": diagnosis_confidence,  # for FST label filtering
            "signals": comps.signals(),
            "complexity": comps.n_checkpoints,  # s_hat, for Fu-style re-rank
            "stored_at": time.time(),
            "retried": False,                  # flipped by the retry phase
        }
        if bucket == "pool":
            record["embedding"] = (embed([question])[0].tolist()
                                   if backend_name() == "minilm" else None)
        with open(path, "a") as f:
            f.write(json.dumps(record) + "\n")
        return path

    # ------------------------------------------------------------- reading
    @staticmethod
    def _load(path: Path) -> list[dict]:
        if not path.exists():
            return []
        with open(path) as f:
            return [json.loads(ln) for ln in f if ln.strip()]

    def _ids(self, path: Path) -> set[str]:
        return {r["id"] for r in self._load(path)}

    def load_pool(self) -> list[dict]:
        """FULL pool records, gold included -- for audit / re-scoring only.
        Do NOT feed these to retrieval or rendering; use exemplars()."""
        return self._load(self.paths["pool"])

    def load_queue(self, which: str) -> list[dict]:
        """which in {'medium', 'bad'} -- in stored (stream) order."""
        return self._load(self.paths[which])

    def counts(self) -> dict[str, int]:
        return {k: len(self._load(p)) for k, p in self.paths.items()}

    # ------------------------------------------------------- Phase-1 hook
    def exemplars(self) -> list[dict]:
        """The GOOD pool, SANITISED for retrieval: each entry has question,
        trace, complexity (s_hat), signals and embedding (or None) -- but
        NEVER gold_solution/gold_answer. This is the function Phase 1's
        retrieval imports; stripping gold here guarantees the exemplar path
        cannot leak the reference solution into any prompt, regardless of
        what the renderer reads. Full records remain available via
        load_pool() for audit."""
        return [_sanitize_exemplar(r) for r in self.load_pool()]