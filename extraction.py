...
# Value-based E/A only ask 'did the right NUMBER appear?'. The structural
# signals (E_struct/A_struct, components.py) ask 'was it produced by the right
# OPERATION on the right OPERANDS?' -- which needs the operands+operators of
# each <<expr=result>> and of each trace equation. Purely additive.
_OP_NORMALISE = {"x": "*", "X": "*"}
_OP_CHARS = set("+-*/xX")
...
def extract_gold_checkpoint_terms(gold_solution: str) -> list:
    ...
    checkpoint's RESULT plus the OPERANDS + OPERATOR(S) gold used to produce
    it (from the <<expr=result>> expr -- capture group 1, which the value-only
    extractor discards)."""
    out = []
    for m in RE_CHECKPOINT.finditer(gold_solution):
...
_RHS = r"\$?\s*-?\d[\d,]*(?:\.\d+)?"
# Unit words may appear INSIDE the expression, not just before '=' -- chat models
# write "50 minutes / 60 = 0.83" and "$12/hour * 0.83 hour = $9.96". Letters are
# therefore allowed in the LHS (bounded, to stop a runaway span across prose) and
# stripped in _clean_expr. _eval_expr's operator + balanced-paren requirements
# reject anything that isn't real arithmetic after cleaning.
RE_EQ_GENERAL = re.compile(
    rf"([0-9(][0-9.,()%$ \t*/+\-x×÷a-zA-Z']{{0,48}}?)\s*=\s*({_RHS})")
_EXPR_ALLOWED = re.compile(r"^[0-9.\s()*/+\-]+$")   # after cleaning
_EXPR_HAS_OP = re.compile(r"[*/+\-]")
# "$12/hour", "60 km/h": a slash followed by a WORD is a unit denominator
# ("per hour"), NOT a division. Drop the slash with it.
_PER_UNIT = re.compile(r"/\s*[a-zA-Z]+")
_WORD = re.compile(r"[a-zA-Z]+")


def _clean_expr(s: str) -> str:
    """Normalise an LHS arithmetic string for sympy: drop $ and thousands commas,
    fold x/× to *, ÷ to /, rewrite 'N%' as '(N/100)', drop per-unit denominators
    ('/hour') and any remaining unit words ('minutes')."""
    s = s.replace("$", "").replace(",", "")
    s = s.replace("×", "*").replace("÷", "/")
    s = _PER_UNIT.sub("", s)          # "12/hour" -> "12"   (before x->* folding)
    s = re.sub(r"(?<=\d)\s*x\s*(?=[\d(])", "*", s)   # "12 x 5" -> "12*5"
    s = _WORD.sub(" ", s)             # strip remaining unit words
...
class Equation:
    ...
    is_true: bool = False
    operands: tuple = ()          # structural E/A: input numbers in the LHS expr
    ops: frozenset = frozenset()  # structural E/A: operator symbols in the LHS expr
...
def extract_final_answer(trace: str) -> Optional[float]:
    ...
    # No explicit answer marker. Fall back to the last number STATED IN THE
    # FINAL non-empty line (the concluding sentence), then to the last number
    # anywhere. Measured on real traces: preferring the last *equation* result
    # here LOSES answers -- the last equation is usually mid-reasoning, while the
    # model's conclusion ("So Jack will get 400 grams") is on the final line.
    for line in reversed([l for l in trace.splitlines() if l.strip()]):
        found = RE_NUMBER.findall(line)
        if found:
            return to_float(found[-1])
