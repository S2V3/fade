...
# cascade) AND R's step list. Default False = one step per non-#### line
# (sentence-splitting only a lone line). Set True to sentence-split EVERY
# line: recovers steps under-counted in terse chat traces -- the ST root
# cause the ST/E gate only mitigates -- but also re-segments R. Behavioural:
# validate on dev before enabling.
...
# The cascade is deterministic, but a trace can sit right on a boundary. Each
# fired rule reports a heuristic confidence in [0,1] = how far INSIDE its
# thresholds the deciding signals are (the min normalised slack). If a TYPED
# diagnosis (NR/ST/SM/CE) fires with confidence < DIAG_MIN_CONFIDENCE it is
# downgraded to UNCLASSIFIED -> generic cure, rather than committing a
# confidently-wrong typed cure. This also keeps low-confidence labels OUT of
# FST's training data (they are noise a predictor would faithfully reproduce).
#
# DEFAULT 0.0 = OFF: confidence is emitted but never triggers a downgrade, so
# behaviour is unchanged. Enable by raising it on DEV (e.g. 0.15-0.25) while
# watching the diagnosis histogram + the UNCLASSIFIED rate. Never tune on test.
