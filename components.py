...
def structural_hits(gold_solution: str, eqs: list) -> tuple:
    ...
    A checkpoint (result r, operands O, ops P) is an E_struct hit if some
    TRUE trace equation has result==r AND the same operand multiset O
    (right operands actually produced r); an A_struct hit if additionally
    its operators cover P. This is the dependency check value-based E/A
    cannot do: landing on r by the wrong route is NOT a structural hit.
    LIMITATION: a chained checkpoint the trace computes in SEPARATE steps
    won't operand-match, so E_struct is a strict LOWER BOUND (its gap vs
    E = coincidental-match inflation + decomposition)."""
...
class ComponentScores:
    ...
    n_equations: int
    r_variant: str
    E_struct: float = 0.0   # structural (right operands) -- LOGGED, not yet driving
